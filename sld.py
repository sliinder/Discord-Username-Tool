...
import sys
init(autoreset=True)
__version__ = "SLinder Username Tool"
__github__= "https://github.com/sliinder"
...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
  ...
  Discord Username's availability validator by SLinder.
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
""")
    proc0()
...
def proc0():
    m_input = input(f"{Fore.LIGHTBLACK_EX}[{Fore.LIGHTGREEN_EX}SLD{Fore.LIGHTBLACK_EX}]:> {Fore.LIGHTYELLOW_EX}").lower()
    if m_input=="exit":
        sys.exit(0)
    if m_input=="":
        proc0()
   ...
   if opt == 2:
    for username in usernames:
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{username}': {endpoint.json()['message']} |SLD: Make sure you have a valid token.")
   elif opt == 1:
       body = {
           "username": usernames
       }
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{usernames}': {endpoint.json()['message']} |SLD: Make sure you have a valid token.")
...
def ch_send_webhook(val0:str):
   if webhook_0 == True:
       ...
       embeds=[
    {
      ...
      "footer": {
        "text": "Made By SLinder (_u6.)"
      },
      "author": {
        "name": "SLD - Username Found",
        "url": "https://github.com/sliinder",
        "icon_url": "https://cdn-icons-png.flaticon.com/512/5290/5290982.png"
      },
      "thumbnail": {
        "url": "https://i.imgur.com/nG52f1y.png"
      },
      "fields": [],
      "color": 16768000
    }
       ...
       print(f"{Lb}[!]{Fore.RED} Error: Something went wrong while sending the webhook request. Exception: {s} | SLD: Make sure you have a valid webhook URL")
   else:
      return
def opt1func(v1,v2):
   for i in range(v1):
